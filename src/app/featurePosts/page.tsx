                ...
                <section className="feature-posts py-10 px-10 text-center">
                    <div className="max-w-7xl mx-auto">
                        <div className="flex flex-col md:flex-row gap-6 mb-10">
                            ...
                            {/* LEFT 60% → First article */}
                            <div className="md:w-3/5 flex flex-col gap-4 bg-white rounded-xl" style={{ boxShadow: '0 5px 25px 0 rgba(114,114,255,.12)' }}>
                                <div className="relative group h-[420px] rounded-2xl overflow-hidden">
                                    <Image
                                    ...
                                    <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-20">
                                        {(Array.isArray(articles[0]?.tag) ? articles[0].tag : [articles[0]?.tag])
                                            .map((t, i) => (
                                                <span
                                                    key={i}
                                                    className="bg-white/20 text-white text-xs font-semibold px-2 py-1 rounded-md uppercase tracking-wide"
                                                >
                                                    {t}
                                                </span>
                                            ))}
                                    ...
                                    {/* Min read on hover */}
                                    <div className="absolute top-3 right-3 flex items-center gap-1 text-white text-xs bg-white/10 px-3 py-1 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontWeight: 700 }}>
                                        <Clock className="w-5 h-5" />
                                        <span style={{ fontSize: '16px' }}>{articles[0]?.readTime} min read</span>
                                    </div>
                                </div>

                                {/* Bottom content */}
                                <div className="mx-6 pt-6 text-left">
                                    <div className="text-sm text-gray-200" style={{ color: '#696981', fontWeight: 600 }}>
                                        <span className="font-medium" style={{ color: '#5955d1', fontWeight: 600 }}>{articles[0]?.author}</span> on {articles[0]?.date}
                                    </div>
                                    <h2 className="text-3xl font-bold text-white my-2" style={{ color: '#29294b', fontSize: '40px' }}>{articles[0]?.title}</h2>
                                    <p className="text-white/90 mb-2" style={{ color: '#696981', fontSize: '1.125rem' }}>{articles[0]?.excerpt}</p>
                                </div>
                            </div>

                            {/* RIGHT 40% → Remaining articles */}
                            <div className="md:w-2/5 flex flex-col gap-4">
                                {articles.slice(1, 4).map((a) => (
                                    <div key={a.id} className="flex gap-4 group rounded-2xl overflow-hidden items-center">

                                        {/* Content → Right 60% */}
                                        <div className="flex-1 text-left flex flex-col justify-center px-2 py-4">
                                            <div className="flex items-center text-sm gap-1" style={{ fontSize: '.937rem', fontWeight: 600 }}>
                                                <span className="font-medium text-indigo-600">{a.author}</span>
                                                <span className="text-gray-500" style={{ color: '#696981' }}>on {a.date}</span>
                                            </div>
                                            <h2 className="py-2 text-lg font-semibold leading-tight" style={{ color: '#29294b', fontWeight: 700 }}>{a.title}</h2>
                                            <p style={{ fontSize: '1rem', fontWeight: 400, color: '#696981', maxWidth: '464px' }}>{a.excerpt}</p>

                                            {/* Tags below content */}
                                            <div className="flex flex-wrap gap-2 mt-8">
                                                {(Array.isArray(a.tag) ? a.tag : [a.tag]).map((t, idx) => (
                                                    <span
                                                        key={idx}
                                                        className="bg-white-200 text-gray-800 text-xs font-semibold px-2 py-1 rounded-md uppercase tracking-wide"
                                                        style={{ boxShadow: '0 5px 25px 0 rgba(114,114,255,.20)' }}
                                                    >
                                                        {t}
                                                    </span>
                                                ))}
                                        ...
                                        {/* Image → Left 40%, fixed size */}
                                        <div className="w-[170px] h-[170px] relative flex-shrink-0 flex justify-center items-center">
                                            <Image
                                                src={a.image}
                                                alt={a.title}
                                                fill
                                            ...
                                            />
                                            <div className="absolute bottom-18 left-8 flex items-center gap-1 text-white text-xs bg-white/10 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition" style={{fontWeight : 700}}>
                                                <Clock className="w-4 h-4" />
                                                <span className="text-sm">{a.readTime} min read</span>
                                            </div>
                        ...
                        <div className="grid grid-cols-1 gap-8">
                            {paginatedArticles.map((a) => (
                                <Link key={a.id} href={`/articles/${a.id}`}>
                                    <article
                                        className="flex flex-row overflow-hidden group justify-center items-center">
                                        {/* Image */}
