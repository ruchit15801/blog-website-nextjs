...
const DesktopMessageRow = ({
    ...
    token: string | null;
    markAsReadAndNavigate: (msg: ContactMessage) => void;
    openEmailModal: (msg: ContactMessage) => void;
}) => {
    const isExpanded = expandedMessages.includes(msg._id);
        ...
        <tr
            ...
            <td className="px-5 py-5 text-gray-700 max-w-md">
                <p className="leading-snug text-sm">
                    {msg.message.length > 120 && !isExpanded ? (
                        <>
    ...
    return (
        <div key={msg._id} className="bg-white rounded-2xl shadow p-4 flex flex-col gap-2">
            ...
            <p className="text-gray-600 text-sm mt-1">
                {msg.message.length > 100 && !isExpanded ? (
                    <>
                        {msg.message.slice(0, 100)}...
                        <button className="text-indigo-500 ml-1" onClick={() => toggleMessage(msg._id)}>
                            Read more
                        </button>
                    </>
                ) : (
...
export default function ContactUsAdminPage() {
    ...
    const markAsReadAndNavigate = async (msg: ContactMessage) => {
    ...
    const openEmailModal = (msg: ContactMessage) => {
    ...
    const loadMessages = useCallback(() => {
        ...
        fetchContactMessages({ token, page, limit, q: searchTerm })
            .then((res) => {
    ...
    const handleSendEmail = async () => {
        ...
        if (!subject.trim() || !emailBody.trim()) return toast.error("Subject and message cannot be empty");
        ...
        try {
            ...
            if (res.success) {
                ...
                setEmailBody("");
                setMessages((prev) =>
                    prev.map((m) => (m._id === selectedMessage._id ? { ...m, status: "read" } : m))
                );
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto mt-4 lg:mt-0">
                        ...
                        <div className="custom-dropdown w-full sm:w-auto relative">
                            <button
                                ...
                                className="flex items-center justify-between w-full sm:w-auto px-3 h-10 rounded-lg border border-gray-300 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400">{limit} / page
                ...
                <div className="hidden lg:block overflow-x-auto">
                    <table className="table-auto w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100 overflow-hidden">
                            {loading && <tr><td colSpan={7} className="px-4 py-6 text-center"><Loader inline label="Loading messages..." /></td></tr>}
                            {error && !loading && <tr><td colSpan={7} className="px-4 py-6 text-center text-red-600">{error}</td></tr>}
                            {!loading && !error && messages.length === 0 && <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-600">No messages found.</td></tr>}
                        ...
                        <div className="relative z-10 w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-6 animate-fadeIn">
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); handleSendEmail(); }} className="flex flex-col gap-4">
                                ...
                                <div className="flex justify-end gap-3">
                                    <button type="button" className="px-5 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition-shadow shadow-sm" onClick={() => setModalOpen(false)}>
                                        Cancel
                                    </button>
                                    <button type="submit" className="px-5 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl hover:scale-105 hover:shadow-lg transition-all shadow-md" disabled={sendingEmail}>
