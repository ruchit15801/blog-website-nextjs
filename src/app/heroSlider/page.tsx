...
export default function HeroSlider() {
    ...
    const articlesFeatures = useCallback(() => {
        setIndex((prev) => (prev === featured.length - 1 ? 0 : prev + 1));
    }, [featured.length]);

    useEffect(() => {
        const timer = setInterval(articlesFeatures, 5000);
        ...
        <>
            <main>
                <section className="pb-6 pt-4 px-10 text-center">
                    {/* Slider wrapper */}
                    <div className="relative max-w-7xl mx-auto overflow-hidden rounded-2xl group h-[620px]">
                        {featured.map((a, i) => (
                            <div
                                key={a.id}
                                className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${i === index ? "opacity-100 z-10" : "opacity-0 z-0"}`}>
                                <Link href={`/articles/${a.id}`}>
                                    <article className="relative w-full h-full rounded-2xl overflow-hidden">
                                        {/* Background image */}
                                        <Image
                                            src={a.image}
                                            alt={a.title}
                                        ...
                                        {/* Tags */}
                                        <div className="absolute p-10 top-3 left-3 flex flex-wrap gap-2 z-20">
                                            {Array.isArray(a.tag)
                                                ? a.tag.map((t, idx) => (
                                                    <span
                                                        key={idx}
                                                ...
                                                : (
                                                    <span className="bg-white/20 text-white text-xs font-semibold px-2 py-1 rounded-md uppercase tracking-wide">
                                                        {a.tag}
                                                    </span>
                                                )}
                                        </div>
                                        ...
                                        <div className="absolute top-10 right-10 flex items-center gap-1 text-white text-xs bg-white/10 px-3 py-1 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontWeight: 700 }}>
                                            <Clock className="w-5 h-5" />
                                            <span style={{ fontSize: '16px' }}>{a.readTime} min read</span>
                                        </div>

                                        {/* Bottom content */}
                                        <div className="absolute bottom-0 p-10 z-20 text-left">
                                            <div className="text-sm text-gray-200 my-2" style={{fontSize : '16px'}}>
                                                <span className="font-medium" style={{color : 'white'}}>{a.author}</span> • {a.date}
                                            </div>
                                            <h2 className="text-3xl font-bold text-white my-2 " style={{ fontSize: '50px' , maxWidth : '940px' }}>
                                                {a.title}
                                            </h2>
                                            <p className="text-white/90 text-base" style={{ fontSize: '20px' ,color : '#c2c2ceff' , maxWidth : '640px'}}>{a.excerpt}</p>

                                            <button className="discover1 mx-auto ms-0 mt-10" style={{ cursor: 'pointer' }}>Discover More</button>
                                        </div>
                                    </article>
                                </Link>
                            </div>
                        ...
                        <button
                            onClick={prev}
                            className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 p-3 rounded-full z-30 opacity-0 group-hover:opacity-100 transition"
                            aria-label="Previous"
                        >
                        ...
                        <button
                            onClick={next}
                            className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 p-3 rounded-full z-30 opacity-0 group-hover:opacity-100 transition"
                            aria-label="Next">
